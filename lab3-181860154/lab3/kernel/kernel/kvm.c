...
int loadElf(const char *filename, uint32_t physAddr, uint32_t *entry) {
	...
	for (i = 0; i < inode.blockCount; i++) {
		res=readBlock(&sBlock, &inode, i, (uint8_t *)(elf + i * sBlock.blockSize));
		if(res==-1)
			return -1;
	}
	...
	*entry = ((struct ELFHeader *)elf)->entry; // entry address of the program
	...
	int phnum = ((struct ELFHeader *)elf)->phnum; // nums of program header
	int phentsize = ((struct ELFHeader *)elf)->phentsize; // size of program header
	//
	for(i=0;i<phnum;i++)
	{
		int phoff = ((struct ELFHeader *)elf)->phoff+i*phentsize; // program header offset
		int type=((struct ProgramHeader *)(elf+phoff))->type;//type
		if(type==1)
		{
			int offset = ((struct ProgramHeader *)(elf + phoff))->off;// .text section offset
			int vaddr = ((struct ProgramHeader *)(elf + phoff))->vaddr;
			int filesz = ((struct ProgramHeader *)(elf + phoff))->filesz;
			int memsz = ((struct ProgramHeader *)(elf + phoff))->memsz;

			for(int j=0;j<filesz;j++)
				*(uint8_t *)(elf+vaddr+j)=*(uint8_t *)(elf+offset+j);
			for(int j=filesz;j<memsz;j++)
				*(uint8_t *)(elf+vaddr+j)=0;
		}
	}
	return 0;
}

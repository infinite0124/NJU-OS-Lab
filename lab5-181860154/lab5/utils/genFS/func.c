...
int rmdir (const char *driver, const char *destDirPath) {
    ...
    *((char*)destDirPath + size + 1) = tmp;
    ...
    while (getDirEntry(file, &superBlock, &inode, dirIndex, &dirEntry) == 0) {
        if(stringCmp(dirEntry.name,"..",2)==0)
        {
            dirIndex++;
            continue;
        }
        ...
        int name_len=stringLen(dirEntry.name);
        char temp[300];
        stringCpy(destDirPath,temp,length);
        *((char*)temp + length) = '/';
        for(int i=0;i<=name_len;i++)
            *((char*)temp + length + 1 + i) = dirEntry.name[i];
        rm(driver,temp);
    }

    //edit father directory
    DirEntry *dirEntry2;
    uint8_t buffer[superBlock.blockSize];
    int i;
    for (i = 0; i < fatherInode.blockCount; i++) {
        ...
        if (ret == -1)
            return -1;
        dirEntry2 = (DirEntry *)buffer;
        int j;
        for (j = 0; j < superBlock.blockSize / sizeof(DirEntry); j++) {
            if (stringCmp(dirEntry2[j].name, destDirPath+size+1, length) == 0)
            {
                //make the target entry invalid
                dirEntry2[j].inode = 0;
                ret = writeBlock(file, &superBlock, &fatherInode, i, buffer);
                if (ret == -1)
                    return -1;
...
int rm (const char *driver, const char *destFilePath) {
    ...
    *((char*)destFilePath + size + 1) = tmp;
    ...
    if (ret == -1) {
        printf("Failed to read dest inode.\n");
        fclose(file);
        return -1;
    }
    //edit father directory
    ...
    for (i = 0; i < fatherInode.blockCount; i++) {
        ...
        int j;
        for (j = 0; j < superBlock.blockSize / sizeof(DirEntry); j++) {
            if (stringCmp(dirEntry[j].name, destFilePath+size+1, length) == 0)
            {
                //make the target entry invalid
                dirEntry[j].inode = 0;
                ret = writeBlock(file, &superBlock, &fatherInode, i, buffer);
    ...
    //edit inodeBitmap and superBlock
    int j = 0;
    int k = 0;
    int inodeBitmapOffset = 0;
    int inodeTableOffset = 0;
    ...
    fseek(file, inodeBitmapOffset * SECTOR_SIZE, SEEK_SET);
    fread((void *)&inodeBitmap, sizeof(InodeBitmap), 1, file);
    int temp = (destInodeOffset - inodeTableOffset * SECTOR_SIZE) / sizeof(Inode);
    j = temp/8;
    k = temp%8;
    inodeBitmap.byte[j] = inodeBitmap.byte[j] & ~(1 << (7 - k));
    fseek(file, inodeBitmapOffset * SECTOR_SIZE, SEEK_SET);
    ...
    //edit blockBitmap and superBlock
    int blockBitmapOffset = 0;
    BlockBitmap blockBitmap;
    ...
    { 
        temp=(destInode.pointer[i]-superBlock.blocks)/(superBlock.blockSize/SECTOR_SIZE);
        j=temp/8;
        k=temp%8;
        int origin=blockBitmap.byte[j];
        blockBitmap.byte[j] = blockBitmap.byte[j] & ~(1 << (7 - k));
        if((origin&(int)blockBitmap.byte[j])!=origin)
        superBlock.availBlockNum ++;
    }
    fseek(file, 0, SEEK_SET);
    fwrite((void *)&superBlock, sizeof(SuperBlock), 1, file);

...
void keyboardHandle(struct TrapFrame *tf) {
	...
	if(keycode!=0)
	{
		...
		assert(bufferTail<MAX_KEYBUFFER_SIZE);
...
void syscallWriteShMem(struct TrapFrame *tf) {
	...
	if(fd==SH_MEM)
	{
		...
		for(int i=0;i<size;i++)
			asm volatile("movb %%es:(%1), %0":"=r"(shMem[index+i]):"r"(buffer + i));
	}
}
...
void syscallReadShMem(struct TrapFrame *tf) {
	...
	if(fd==SH_MEM)
	{
		asm volatile("movw %0, %%es"::"m"(sel));
		for(int i=0;i<size;i++)
			asm volatile("movb %0, %%es:(%1)"::"r"(shMem[index+i]),"r"(buffer + i));
	}
}
...
void syscallSemDestroy(struct TrapFrame *tf) {
	...
	if(sem[i].state==1)
	{
		if(sem[i].pcb.next!=&sem[i].pcb||sem[i].pcb.prev!=&sem[i].pcb)
			pcb[current].regs.eax=-1;
		else
		{
			sem[i].state=0;
